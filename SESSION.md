...
2026-09-09: 依存を **typescript 7.0.2 / vite 8.2.2** へ更新 (型検査・ビルド・テスト全通過、
公開版も更新済み)。branch は `master` のまま維持する判断 — Pages がそこから配信しており、
rename は Pages source 更新と同 turn でないと配信が切れるため。
⚠️ このとき `vite build` の `gzip:` 表示を実測値と取り違えて更新を見送りかけた
(詳細と再発防止は CLAUDE.md §配布)。
