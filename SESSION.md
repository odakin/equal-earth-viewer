...
- **方位図法は中心緯度も可変** (`lat` state / URL、地図右脇の縦スライダー 〔上が北〕 + 数値 + 極プリセット + 縦ドラッグ、方位のみ表示。verify A2)
- **図法選択をプルダウンからボタン列へ** (族ごとに 1 行、user 要望)
- **図法を 14 種に拡張** (代表的な正積図法、族別 optgroup。グード断裂は回すと大陸が切れるので一度固定化して入れたが趣旨違いで削除 = DESIGN.md。正積であることを verify.mjs F がティソー円面積比で実測。DESIGN.md)
