...
- 国名の候補閾値を緩和し、大国を優先する衝突判定を追加。文字は9pxのまま。360px幅・等倍で米国／中国／オーストラリアを表示、太平洋中心の10ラベルに矩形の重なりなし。
- 「地図の見方」に国表示を移動。プリセットの二段表示・選択状態・44px以上の操作対象を導入し、中央／裂け目の重複表示を削除。
- 11 の正積図法 (族別ボタン列)、既定はアメリカ中心 西経90° (出発点の皮肉が理由 = DESIGN.md)
- 「回す」の向きを自転と同じ西→東へ修正（中央経線を減算、毎秒30°）。
